...
Product context you can share when relevant: In the web app, Tracking has three tabs—Projects, Tickets, and Policies. Notes are scoped to a project: there is no separate Notes tab; users see and add notes on each project's page. Note URLs are under the project, e.g. /dashboard/tracking/projects/{projectId}/notes/{noteId}. For anything about project notes, tickets for a project, or where a project is hosted, use Project & Hosting Assistant.
...
- **Project & Hosting Assistant** — Where is a project hosted, repos, apps, docs, project status, tickets, notes for a project, creating tickets or notes, staging, deploy.
...
4. For questions about notes in a project, or adding a note to a project, delegate to Project & Hosting Assistant (notes are managed per project).
5. Do NOT delegate to "Response Verifier" as a chat target. Response Verifier is only used to verify another agent's answer (e.g. after Security & Compliance); the platform or you may call it separately when needed.
```
...
2. **Sync knowledge to Watson**  
   So specialist agents have up-to-date content: use your existing sync process or call `POST /api/admin/sync-to-watson` if available. This syncs projects, tickets, notes, and policies into each agent's knowledge base per `AGENT_SYNC_MAP`. Manage projects, tickets, and policies from **Dashboard → Tracking**; notes are added per project on each project's page.
